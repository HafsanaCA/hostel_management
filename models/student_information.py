...
class StudentInformation(models.Model):
    ...
    invoice_status = fields.Selection([('pending', 'Pending'), ('done', 'Done')],string="Invoice Status",
                                      compute='_compute_invoice_details',store=True)
    pending_amount = fields.Float(string="Pending Amount", compute="_compute_invoice_details", store=True)
    ...
    def _compute_invoice_details(self):
        """Computes the invoice status and total pending amount of each student"""
        for student in self:
            pending_invoices = student.invoice_ids.filtered(lambda inv: inv.state not in ['posted', 'paid'])
            student.pending_amount = sum(pending_invoices.mapped('amount_total'))
            student.invoice_status = 'pending' if pending_invoices else 'done'
